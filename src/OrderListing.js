    ...
    return (
      <div>
        <Container>
            ...
            <Col s={12} l={8} xl={8} className="offset-l2 offset-xl2">
              <Row>
                <Col s={6} l={4} xl={4} >
                  ...
                  <div className="subtotal-left">{parseFloat(this.state.data['totalBCH']).toFixed(8)} STAK</div>
                </Col>
                <Col s={6} l={4} xl={4} >
          ...
          <Row>
            <Col s={12} l={8} xl={8} className="offset-l2 offset-xl2">
              {this.state.tx !== 'NoTX' ? <Button large waves='light' className='left green darken-2  bold-big' node='a' target='_blank' href={"https://straks.info/transaction/" + this.state.tx}><Icon left>payment</Icon>payment</Button> : <h4 className="left">No payment found!</h4>}
            </Col>
          </Row>
          <Row>
            <Col s={12} l={8} xl={8} className="offset-l2 offset-xl2">
                  ...
                  return (
                    <Row className="detail-item" key={key}>
                      ...
                      <Col s={6} className="offset-l0 offset-xl0">
                        ...
                        <span className="left mono-tiny">{item.bch} STAK</span><br />
                      ...
                      <Col s={4} className="offset-l0 offset-xl0">
                          ...
                          <span className="right mono-tiny">{parseFloat(item.count * item.bch).toFixed(8)} STAK</span>
                      </Col>
                    </Row>
                  );
                })}
            ...
            <Col s={12} l={8} xl={8} className="offset-l2 offset-xl2">
              <Row>
                ...
                <Col s={5} l={3} xl={3} className="">
                  <div className="subtotal-right mono-tiny">{parseFloat(this.state.data['subtotalBCH']).toFixed(8)} STAK</div>
                  <div className="subtotal-right mono-tiny">- {parseFloat(this.state.data['discountBCH']).toFixed(8)} STAK</div>
                  <div className="subtotal-right mono-tiny">{(parseFloat(this.state.data['subtotalBCH'] - this.state.data['discountBCH'])).toFixed(8)} STAK</div>
                  <div className="subtotal-right mono-tiny">+ {parseFloat(this.state.data['taxBCH']).toFixed(8)} STAK</div>
                  <div className="subtotal-right bold">{parseFloat(this.state.data['totalBCH']).toFixed(8)} STAK</div>
                </Col>
