            ...
            <tbody>{this.state.data.map((item, key) => {
              return (
                <tr key = {key}>
                  ...
                  <td><a target="_blank" href={"https://straks.info/en/address/" + item.addr}>{item.addr.slice(0,18) + "..." + item.addr.slice(-7,-1)}</a></td>
                  <td>{item.amount}</td>
                  <td>{item.label}</td>
                  {
                  //<td>{item.received ? <a target="_blank" href={"https://straks.info/en/transaction/" + item.txid}><Icon className="green-text">check</Icon></a> : <Icon className="red-text">clear</Icon>}</td>
                  }
                  <td>{item.received ? <Button waves='light' className='grey lighten-2' node='a' target='_blank' href={"https://straks.info/en/transaction/" + item.txid}><Icon className="green-text">check</Icon></Button> : <Icon className="red-text">clear</Icon>}</td>
